<?php
  class funciones{
    ...
    public function registrarCita($codigo,$nom,$entidad,$tipo,$fechaI,$fechaS,$fechaA,$observa,$medico){/**/
        mysql_query("INSERT INTO citas (cedula,nombre,entidad,tipo,fechaSolicitud,fechaAsignacionPaciente,fechaAsignacionSistema,observaciones,doctor)
                                      VALUES ('$codigo','$nom','$entidad','$tipo','$fechaI','$fechaS','$fechaA','$observa','$medico')")
                                      or die ("Error");
    }
    ...
    public function verCitas(){
        $resultado = mysql_query("SELECT * FROM citas,medicos WHERE citas.doctor=medicos.id");
        while($fila = mysql_fetch_array($resultado)){
            echo '<tr> 
                    <td>'.$fila['cedula'].'</td>
                    <td>'.$fila['nombre'].'</td>
                    ...
                    <td><a id="editarMedico" class="btn btn-mini btn-info" href="'.$fila['id'].'"><strong>Editar</strong></a></td>
                </tr>';
        }
    }
    ...
    public function modificarMedico($cod,$nombre){
        mysql_query("UPDATE medicos SET nombreMedico='$nombre' WHERE id='$cod'")
   ...
   /* public function registrarGasto($gasto,$concepto,$fecha){
    ...
    public function notificarFecha(){
        ...
        while($fila = mysql_fetch_array($resultado)){
            ...
            $dia2 = $dia + 5;//colocar mas dias para las quincenas
            if($fila['tipo'] == 'm'){
   ...
   /*aca comienzo con la partde de actulizar datos de los estudiantes que van al gim*/
    ...
    public function comboMedicos(){
        ...
        while ($fila = mysql_fetch_array($result)) {
            echo "<option value='".$fila['id']."'>".$fila['nombreMedico']."
                     </option>";
        }
    }
